...
numa_topo::numa_topo(const hwloc_topology_t topology)
    : topo_(topology), distance_matrix_(get_hwloc_distance_matrix()) {
}

numa_topo::pu_matrix_t
numa_topo::get_pu_matrix(const pu_set_t& current_pu_id_set) const {
  ...
  node_id_t num_of_dist_objs = distance_matrix_->nbobjs;
  float* dist_pointer =
...
numa_topo::node_set_t numa_topo::get_node_set(const pu_set_t& pu_set) const {
  node_set_t node_set = hwloc_bitmap_make_wrapper();
  hwloc_cpuset_to_nodeset(topo_, pu_set.get(), node_set.get());
  return node_set;
}

numa_topo::pu_set_t numa_topo::get_pu_set(const node_set_t& node_set) const {
  pu_set_t pu_set = hwloc_bitmap_make_wrapper();
  hwloc_cpuset_from_nodeset(topo_, pu_set.get(), node_set.get());
  return pu_set;
}

const hwloc_distances_s* numa_topo::get_hwloc_distance_matrix() const {
